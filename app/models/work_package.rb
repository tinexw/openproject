...
class WorkPackage < ApplicationRecord
  ...
  # The configuration to read from. A type carries none of its own: this work package stores
  # its type, and its project decides which of that type's variants applies. Use this for form
  # configuration, workflows, custom fields, defaults and export templates; use #type for
  # identity, filtering and grouping.
  def type_variant
  ...
  # A work package stores its type, while its project may apply a variant owning a different
  # form configuration. The fields available therefore depend on the (project, type) pair
  # rather than the type alone, which is what #available_custom_field_key already caches on.
  #
  ...
  # Match custom fields on the variant that owns the (possibly linked) form configuration, minus
  # the ones its chain excludes, but keep the driving variant id available so a batch preload
  # can match it against each work package's effective variant id.
