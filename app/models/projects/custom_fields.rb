...
module Projects::CustomFields
  ...
  included do
    ...
    def available_custom_fields_for_variant(variant_id)
      ...
      # A variant Linked for PROJECT_ATTRIBUTES has no own mappings; its enabled attributes are
      # the source variant's. The chain is resolved inline via a subquery so no extra query is
      # issued per call.
