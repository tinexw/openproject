...
# The identity of a kind of work package: what it is called, how it looks, and whether it
# behaves as a milestone. A type carries no configuration of its own — the form, workflows,
# subject patterns, PDF export settings and project attributes all live on its variants, and
# which variant applies is decided per project (see ProjectType).
class Type < ApplicationRecord
  include ::Scopes::Scoped

  # A type is nothing without a configuration to apply, so it always has a base variant.
  # Built before validation (and autosaved) so assigning projects on create can resolve
  # ProjectType#variant to the base configuration in the same save.
