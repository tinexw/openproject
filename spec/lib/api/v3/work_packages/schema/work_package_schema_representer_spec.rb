...
RSpec.describe API::V3::WorkPackages::Schema::WorkPackageSchemaRepresenter do
  ...
  describe "caching" do
    ...
    context "for a TypedWorkPackageSchema" do
      ...
      it "does not cache the attribute_groups" do
        ...
        # Rendering two times, the TypeVariant#attribute_groups
  ...
  # A work package stores the type, so the representer is always handed that type and
  # has to resolve the configuration in force itself. Every example below configures the base
  # and the named variant differently, so following the stored type alone is a visible failure.
  describe "when the project resolves the type to a variant", with_flag: { type_variants: true } do
    ...
    shared_let(:variant) do
      create(:type_variant, type: family_root, variant_name: "Variant").tap do |named|
        # Named variants start independent; link them so an aspect only differs once dropped.
    ...
    # Pin the resolved variant: parent examples stub `type_variant`, and build_stubbed
    # work packages keep stale project_types associations across shared_let updates.
    let(:work_package) do
    ...
    describe "caching" do
      ...
      # timestamp, so without the type variant in the key the previous form configuration would
      # keep being served.
