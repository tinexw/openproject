...
RSpec.describe "Project settings work package types", :js, with_flag: { type_variants: true } do
  ...
  # The row names the variant the project applies, so the removal is requested with the
  # variant's id while the row that has to go is the type's.
  it "removes a type the project runs through a named variant" do
  ...
  it "offers no switch action for a type without named variants" do
