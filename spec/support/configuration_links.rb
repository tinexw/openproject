...
# A configuration link is a pair of columns per aspect rather than a row, so there is no
# factory for one. These write that pair, taking a type wherever a variant is expected so
# that `link_configuration(a_type, source: another_type, aspect:)` reads the same as before.
module ConfigurationLinkHelpers
  ...
  # write-time prevention (FND-133), for the specs that assert resolution terminates anyway.
  ...
  # Only an excludable aspect has the column to write.
