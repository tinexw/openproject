...
RSpec.describe WorkPackageTypes::CopyConfiguration::FormConfigurationService do
  ...
  # Switching to Independent copies from the variant's own link source, so the copy has to freeze
  # what the variant was presenting — not restore the elements its link chain excluded.
  describe "when the variant's link excludes elements", with_flag: { type_variants: true } do
    ...
    # afterwards — and TypeVariant#attribute_groups resolves through a live link. Reading it
    # directly would therefore show the still-inherited view and hold whatever was actually
    # persisted, so these assertions sever the link first and look at the variant's own stored
    # groups.
    def own_groups
