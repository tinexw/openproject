...
RSpec.describe OpenProject::Common::BorderBoxListCollectionComponentPreview, type: :component do
  ...
  around do |example|
    ...
    already_present = previews_config.paths.include?(lookbook_previews_path)
    previews_config.paths << lookbook_previews_path unless already_present

    example.run
  ensure
    previews_config.paths.delete(lookbook_previews_path) unless already_present
  end
